...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--dp-devices"}, "i,j,k,...",
        "comma-separated GPU placement for data-parallel replicas. each comma-separated token is\n"
        "one replica; repeats oversubscribe a GPU (e.g. 0,0,1,1). join GPUs with '+' to place one\n"
        "replica across them via model split (e.g. 0+1,2+3 = two replicas, each spanning 2 GPUs -\n"
        "the model-too-big escape hatch; see --dp-split). empty (default) pins replicas to 0..N-1",
        [](common_params & params, const std::string & value) {
            // each comma-separated token is one replica; a '+' inside a token splits that single
            // replica across the listed GPUs. all-singleton input keeps the historical flat form
            // (fills dp_devices); any '+' fills dp_device_groups instead. the consumer (orchestrator
            // helper) reconciles which one is set.
            const auto tokens = string_split<std::string>(value, ',');
            std::vector<std::vector<int>> groups;
            bool grouped = false;
            for (const auto & tok : tokens) {
                groups.push_back(string_split<int>(tok, '+'));
                if (groups.back().size() > 1) {
                    grouped = true;
                }
            }
            if (grouped) {
                params.dp_device_groups = std::move(groups);
            } else {
    ...
    add_opt(common_arg(
        {"--dp-split"}, "{layer,row,tensor}",
        "how to split a multi-GPU data-parallel replica across its GPUs (only applies to grouped\n"
        "--dp-devices like 0+1,2+3), one of:\n"
        "- layer (default): split layers and KV across the group (pipelined)\n"
        "- row: split layers and KV, using tensor parallelism if supported (parallelized)\n"
        "- tensor: split weights and KV across the group (parallelized, EXPERIMENTAL)",
        [](common_params & params, const std::string & value) {
            if (value == "layer") {
                params.dp_split_mode = LLAMA_SPLIT_MODE_LAYER;
            } else if (value == "row") {
                params.dp_split_mode = LLAMA_SPLIT_MODE_ROW;
            } else if (value == "tensor") {
                params.dp_split_mode = LLAMA_SPLIT_MODE_TENSOR;
            } else {
                throw std::invalid_argument("--dp-split must be 'layer', 'row', or 'tensor'");
            }
        }
    ).set_env("LLAMA_ARG_DP_SPLIT"));
    add_opt(common_arg(
        {"--dp-num-devices"}, "D",
        "data-parallel placement by count: use GPUs 0..D-1 (alternative to listing --dp-devices).\n"
        "combine with --dp-replicas-per-device to oversubscribe each GPU.",
        [](common_params & params, int value) {
            if (value < 1) {
                throw std::invalid_argument("--dp-num-devices must be >= 1");
            }
            params.dp_num_devices = value;
        }
    ).set_env("LLAMA_ARG_DP_NUM_DEVICES"));
    add_opt(common_arg(
        {"--dp-replicas-per-device"}, "R",
        "data-parallel oversubscription: place R replicas on each device selected by\n"
        "--dp-num-devices (default 1). e.g. --dp-num-devices 4 --dp-replicas-per-device 2 = 8 replicas.",
        [](common_params & params, int value) {
            if (value < 1) {
                throw std::invalid_argument("--dp-replicas-per-device must be >= 1");
            }
            params.dp_replicas_per_device = value;
        }
